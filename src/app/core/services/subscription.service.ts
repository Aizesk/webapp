...
export class SubscriptionService {
  ...
  readonly usagePercentage = computed(() => {
    ...
    return sub.usage?.usagePercentage ?? 0;
  });
  ...
  readonly transactionsRemaining = computed(() => {
    ...
    return sub.usage?.remainingTransactions ?? 0;
  });
  ...
  readonly platformsRemaining = computed(() => {
    ...
    return Math.max(0, sub.usage.platformLimit - sub.usage.platformsConnected);
  });

  readonly transactionsUsed = computed(() => {
    return this._currentSubscription()?.usage?.transactionsUsed ?? 0;
  });

  readonly platformsConnected = computed(() => {
    return this._currentSubscription()?.usage?.platformsConnected ?? 0;
  });
  ...
  deletePaymentMethod(paymentMethodId: string): Observable<MessageResponse> {
    return this.http
      .delete<MessageResponse>(`${this.apiUrl}/payment-methods/${paymentMethodId}`)
      .pipe(
        tap(() => {
          this._paymentMethods.update((methods) => methods.filter((m) => m.id !== paymentMethodId));
        }),
      );
  }
  ...
  setDefaultPaymentMethod(paymentMethodId: string): Observable<MessageResponse> {
    return this.http
      .put<MessageResponse>(`${this.apiUrl}/payment-methods/${paymentMethodId}/default`, {})
      .pipe(
        tap(() => {
