...
export class PlatformConnectionService {
  ...
  loadPlatforms(): void {
    ...
    this.http
      .get<AvailablePlatform[]>(`${this.apiUrl}/available`)
      .pipe(
        catchError((err) => {
      ...
      .subscribe((available) => {
        ...
        this.http
          .get<PlatformConnection[]>(`${this.apiUrl}/connections`)
          .pipe(
            catchError((err) => {
  ...
  /**
   * Handle OAuth callback — sends code+state to backend generic POST callback.
   * Used when eBay (or other platforms) redirect to the frontend with code/state params.
   */
  handleOAuthCallback(code: string, state: string): Observable<PlatformConnection> {
    return this.http
      .post<PlatformConnection>(`${this.apiUrl}/callback`, {
        code,
        state,
      })
      .pipe(tap(() => this.loadPlatforms()));
  }

  disconnect(connectionId: string): Observable<{ message: string }> {
    return this.http
      .delete<{ message: string }>(`${this.apiUrl}/connections/${connectionId}`)
      .pipe(tap(() => this.loadPlatforms()));
  }
  ...
  sync(connectionId: string): Observable<SyncLogResponse> {
    ...
    return this.http
      .post<SyncLogResponse>(`${this.apiUrl}/connections/${connectionId}/sync`, {})
      .pipe(
        tap((result) => {
